...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
     ;lab 7 code here
HIGH
	LDR R1, =SSI0_SR_R ;LOAD ADDRESS
	LDR R2, [R1] ;READ ADDRESS
	AND R2, #0x10 ;MASK BIT 4
	CMP R2, #0x10
	BEQ HIGH ;IF BIT 4 IS HIGH LOOP
	...
	;WRITE THE 8 BIT COMMAND TO SSIO_DR_R
	LDR R1, =SSI0_DR_R ;LOAD ADDRESS
	STR R0, [R1] ;STORE R0 INTO SSI0_DR_R

	;CHECKS TO SEE IF WE CAN LEAVE THE SUBROUTINE
HIGH2
	LDR R1, =SSI0_SR_R
	LDR R2, [R1] ;READ
	AND R2, #0x10 ;MASK BIT 4
	CMP R2, #0x10 ;COMPARES IF R2 == 1
	BEQ HIGH2 ;IF BIT 4 IS HIGH LOOP
...
LOW
	...
	CMP R2, #0x02 ;COMPARES IF R2 BIT 4 == 0x02
	BNE LOW ;IF BIT 1 IS LOW LOOP
	...
	;WRITE THE 8 BIT DATA COMMAND TO SSIO_DR_R
	LDR R1, =SSI0_DR_R ;LOAD ADDRESS
	STR R0, [R1] ;STORE R0 INTO SSI0_DR_R
